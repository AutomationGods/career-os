...
export interface DocumentExportStore {
  ...
  getById(id: string, currentUserId?: string): Promise<DocumentExportRecord | undefined> | DocumentExportRecord | undefined;
...
function normalizeExportRow(row: unknown): Pick<DocumentExportRecord, "id" | "userId" | "documentType" | "format" | "url" | "createdAt"> | undefined {
  if (!row || typeof row !== "object") return undefined;
  const record = row as { id?: unknown; userId?: unknown; documentType?: unknown; format?: unknown; url?: unknown; createdAt?: unknown };
...
export class InMemoryDocumentExportStore implements DocumentExportStore {
  ...
  getById(id: string, currentUserId?: string) {
    ...
    if (!record || (currentUserId && record.userId !== currentUserId && record.metadata?.userId !== currentUserId)) return undefined;
    return record;
  }
  ...
  list(input: DocumentExportListInput = {}) {
    ...
    return [...this.exports.values()]
      .filter((record) => !input.userId || record.userId === input.userId || record.metadata?.userId === input.userId)
...
export class PrismaDocumentExportStore implements DocumentExportStore {
  ...
  async save(input: DocumentExportSaveInput) {
    ...
    if (this.client.documentExport && this.client.documentVersion && this.client.documentMetadata) {
      const exportRow = await this.client.documentExport.create({ data: { id, userId: input.userId, documentType: input.documentType, format: input.format, url, createdAt } });
      await this.client.documentVersion.create({ data: { id: `document_version_${id}`, userId: input.userId, documentType: input.documentType, documentId: id, content, createdAt } });
      await this.client.documentMetadata.create({ data: { id: `document_metadata_${id}`, userId: input.userId, documentType: input.documentType, documentId: id, metadata } });
    ...
    return this.rawSave({ id, userId: input.userId, createdAt, url, documentType: input.documentType, format: input.format, content, metadata });
  }
  ...
  async getById(id: string, currentUserId?: string) {
    if (this.client.documentExport) {
      ...
      if (!exportRow || (currentUserId && exportRow.userId !== currentUserId)) return undefined;
      const versionRow = this.client.documentVersion ? await this.client.documentVersion.findFirst({ where: { documentType: exportRow.documentType, documentId: id, ...(currentUserId ? { userId: currentUserId } : {}) }, orderBy: { createdAt: "desc" } }) : undefined;
      const metadataRow = this.client.documentMetadata ? await this.client.documentMetadata.findFirst({ where: { documentType: exportRow.documentType, documentId: id, ...(currentUserId ? { userId: currentUserId } : {}) } }) : undefined;
      return { ...exportRow, content: normalizeVersionContent(versionRow), metadata: normalizeMetadata(metadataRow) };
    }

    return this.rawGetById(id, currentUserId);
  }

  async list(input: DocumentExportListInput = {}) {
    const limit = input.limit ?? 50;
    const rows = this.client.documentExport
      ? await this.client.documentExport.findMany({ take: limit * 3, orderBy: { createdAt: "desc" }, where: { ...(input.format ? { format: input.format } : {}), ...(input.userId ? { userId: input.userId } : {}) } })
    ...
    for (const row of rows) {
      ...
      const record = await this.getById(normalized.id, input.userId);
      if (record) hydrated.push(record);
    }
    return hydrated.filter((record) => !input.userId || record.userId === input.userId || record.metadata?.userId === input.userId).slice(0, limit);
  }

  private async rawSave(input: { id: string; userId?: string; createdAt: Date; url: string; documentType: string; format: ResumeExportFormat; content: DocumentExportContent; metadata: DocumentExportMetadata }) {
    ...
    const exportRows = await this.client.$queryRawUnsafe<unknown[]>(`INSERT INTO "DocumentExport" (id, "userId", "documentType", format, url, "createdAt") VALUES ($1, $2, $3, $4, $5, $6) RETURNING *`, input.id, input.userId, input.documentType, input.format, input.url, input.createdAt);
    await this.client.$queryRawUnsafe(`INSERT INTO "DocumentVersion" (id, "userId", "documentType", "documentId", content, "createdAt") VALUES ($1, $2, $3, $4, $5::jsonb, $6)`, `document_version_${input.id}`, input.userId, input.documentType, input.id, JSON.stringify(input.content), input.createdAt);
    await this.client.$queryRawUnsafe(`INSERT INTO "DocumentMetadata" (id, "userId", "documentType", "documentId", metadata) VALUES ($1, $2, $3, $4, $5::jsonb)`, `document_metadata_${input.id}`, input.userId, input.documentType, input.id, JSON.stringify(input.metadata));
    const normalized = normalizeExportRow(exportRows[0]);
    if (!normalized) throw new Error("Failed to persist document export.");
    return { ...normalized, content: input.content, metadata: input.metadata };
  }

  private async rawGetById(id: string, currentUserId?: string) {
    if (!this.client.$queryRawUnsafe) return undefined;
    const exportRows = currentUserId
      ? await this.client.$queryRawUnsafe<unknown[]>(`SELECT * FROM "DocumentExport" WHERE id = $1 AND "userId" = $2 LIMIT 1`, id, currentUserId)
      : await this.client.$queryRawUnsafe<unknown[]>(`SELECT * FROM "DocumentExport" WHERE id = $1 LIMIT 1`, id);
    const exportRow = normalizeExportRow(exportRows[0]);
    if (!exportRow) return undefined;
    const versionRows = currentUserId
      ? await this.client.$queryRawUnsafe<unknown[]>(`SELECT * FROM "DocumentVersion" WHERE "documentType" = $1 AND "documentId" = $2 AND "userId" = $3 ORDER BY "createdAt" DESC LIMIT 1`, exportRow.documentType, id, currentUserId)
      : await this.client.$queryRawUnsafe<unknown[]>(`SELECT * FROM "DocumentVersion" WHERE "documentType" = $1 AND "documentId" = $2 ORDER BY "createdAt" DESC LIMIT 1`, exportRow.documentType, id);
    const metadataRows = currentUserId
      ? await this.client.$queryRawUnsafe<unknown[]>(`SELECT * FROM "DocumentMetadata" WHERE "documentType" = $1 AND "documentId" = $2 AND "userId" = $3 LIMIT 1`, exportRow.documentType, id, currentUserId)
      : await this.client.$queryRawUnsafe<unknown[]>(`SELECT * FROM "DocumentMetadata" WHERE "documentType" = $1 AND "documentId" = $2 LIMIT 1`, exportRow.documentType, id);
